...
bool SXBSchDoc::resetSelect_1(bool bDel)
{
	...
	if(m_bTempNewPlace){
		...
		if(!bDel && !m_bTempDirty){	//選択したが、特になにも変更を加えなかった場合
			pObjIte = m_listTemp.begin();
			while(pObjIte != m_listTemp.end()){
...
bool SXBSchDoc::findStringCompare(const char* szTarget,const char* sz,bool bMatchWhole)
{
...
bool SXBSchDoc::findStart(const char* sz,bool bMatchWhole,unsigned int targetFlags)
{
	resetSelect();
	m_strFind = sz;
			...
			switch(nId){
				case SXBSchObj::ID_COMPONENT:
					if(m_nTargetFlags & FINDTGT_NAME){
						if(findStringCompare(((SXBSchComponent*)pObj)->name(),m_strFind.c_str(),m_bFindMatchWhole)){
							pObjSelected = pObj;
							break;
						}
					}
					if(m_nTargetFlags & FINDTGT_NAMELIB){
						if(findStringCompare(((SXBSchComponent*)pObj)->orgname(),m_strFind.c_str(),m_bFindMatchWhole)){
							pObjSelected = pObj;
							break;
						}
					}
					if(m_nTargetFlags & FINDTGT_NUM){
						if(findStringCompare(((SXBSchComponent*)pObj)->refnum(),m_strFind.c_str(),m_bFindMatchWhole)){
							pObjSelected = pObj;
							break;
						}
					}
				...
				case SXBSchObj::ID_TAG:
					if(m_nTargetFlags & FINDTGT_TAG){
						if(findStringCompare(((SXBSchTag*)pObj)->text(),m_strFind.c_str(),m_bFindMatchWhole)){
							pObjSelected = pObj;
							break;
						}
					}
				...
				case SXBSchObj::ID_LABEL:
					if(m_nTargetFlags & FINDTGT_LABEL){
						if(findStringCompare(((SXBSchLabel*)pObj)->text(),m_strFind.c_str(),m_bFindMatchWhole)){
							pObjSelected = pObj;
							break;
						}
					}
				...
				case SXBSchObj::ID_COMMENT:
					if(m_nTargetFlags & FINDTGT_COMMENT){
						if(findStringCompare(((SXBSchComment*)pObj)->text(),m_strFind.c_str(),m_bFindMatchWhole)){
							pObjSelected = pObj;
							break;
						}
					}
